...
export function useContract() {
  ...
  const fetchBets = useCallback(async () => {
    ...
    try {
      ...
      const betPromises = Array.from(
        { length: Number(betCount) },
        async (_, i) => {
  ...
  const fetchStakes = useCallback(async () => {
    ...
    try {
      ...
      const stakePromises = Array.from(
  ...
  const fetchRaffles = useCallback(async () => {
    ...
    try {
      ...
      const rafflePromises = Array.from(
        { length: Number(raffleCount) },
        async (_, i) => {
          try {
            ...
            return {
              id: i,
              creator:
                raffle[0] || "0x0000000000000000000000000000000000000000",
