...
export default function StakeCard({ stake, userAddress }) {
  ...
  const handleStake = async (e) => {
    ...
    try {
      ...
      const success = await stakeInPool(stake.id, stakeAmount);
      if (success) {
        setStakeAmount("");
        notify({
          title: "Success",
      ...
      // Error is already handled in the stakeInPool function
    }
  };

  const handleUnstake = async () => {
    try {
      const success = await unstake(stake.id);
      if (success) {
        notify({
          title: "Success",
      ...
      // Error is already handled in the unstake function
    }
  };

  const canStake =
    stake.active && (!stake.userStake || !stake.userStake.active);
  const canUnstake =
    stake.userStake?.active &&
    BigInt(stake.userStake?.startTime || 0) + BigInt(stake.duration) <=
        ...
        <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                <span className="truncate">
                  {stake.creator === userAddress
                    ? "Created by you"
                    : `${stake.creator.slice(0, 6)}...${stake.creator.slice(
                        -4
                      )}`}
                </span>
              </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">
                {totalStaked.toFixed(4)} / {maxStake.toFixed(4)} ETH
              </span>
            </div>
            ...
            <div className="p-3 bg-secondary/50 rounded-lg">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium">
                  {Number(formatEther(BigInt(stake.userStake.amount))).toFixed(
                    4
                  )}{" "}
                  ETH
                </span>
          ...
          {canUnstake && (
            ...
            >
              {isUnstakeLoading
                ? "Unstaking..."
                : `Unstake ${Number(
                    formatEther(BigInt(stake.userStake.amount))
                  ).toFixed(4)} ETH`}
            </Button>
          )}
          ...
          {stake.userStake?.active && !canUnstake && (
            <p className="text-sm text-center text-muted-foreground">
              ...
              {Math.ceil(
                (Number(
                  BigInt(stake.userStake.startTime) + BigInt(stake.duration)
                ) -
                  Date.now() / 1000) /
                  (24 * 60 * 60)
              )}{" "}
